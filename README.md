...
- 문유주
  - Django와 Vue를 한번에 사용한 프로젝트는 처음이라서 하나의 기능을 구현할 때마다 여러번 검토가 필요했습니다. 잘 되는 것 같다가도 하나의 기능을 작동 시킬 때 다른 기능들도 연쇄적으로 작동하는 것이 많아 한번 오류가 생기면 전역적으로 오류가 생기는 경우가 많았습니다. 이러한 문제들을 팀원과 함께 디버깅해나감으로써 조금 더 시간을 단축할 수 있었으며 
  - Server와 Client의 소통이 중요하다는 것을 느꼈습니다. Server는 Client에게 명세에 맞는 정확한 정보를 제공해야 하고 Client에서는 Server가 요구하는 방식에 맞게 이를 제공해야하며 사용자에게 적절한 방식으로 표현해야 합니다. 이를 위해서는 각자 맡은 역할간의 원활한 의사소통이 필요하며 이번 프로젝트에서도 활발히 소통하여 명세에 맞는 사항을 적절히 구현할 수 있었습니다.
  - 배운 내용들을 최대한으로 이용하여 사이트를 구현하고자 했습니다. 실제로 지금까지의 관통프로젝트에서 다뤘던 내용들을 상당부분 재구현하여 다시한번 되짚어 볼 수 있었던 좋은 기회가 되었습니다.
